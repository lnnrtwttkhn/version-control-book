...
- Go to one of your group's repos
- Create an issue and tag your coding buddy (you might need to ask for their GitHub username)
- Browse issues and comment on issues in other repositories

In addition:

- Link to an `.md` document that you created in one of the previous chapters.
- Create a new label and apply it
- Add new Issues to a Project (create a new Project, if needed)
- Close an Issue

Have fun! And please share your experience with the group.

## Overview

GitHub is famous for helping people work together on code.
But to work well together, you need to talk and organize tasks.
GitHub makes this easy with its feature called [GitHub Issues](https://github.com/features/issues).
...
Recall that we are repurposing Git, a tool that facilitates software development. 
Think of the issues for a project as its bug tracker. 
For projects that are not pure software development, we can co-opt this machinery to organize our to-do list more generally. 
...
If you want more guidance on making a repository, refer to the chapter called [First Steps](first-steps-git.qmd)
...
GitHub allows you to use Markdown syntax in your issue comments, which provides an easy way of formatting text

E.g.: 
Create headers using the # symbol. For example, `# Header 1`, `## Header 2`, and so on.
...
Every repository om GitHub has Issues.
