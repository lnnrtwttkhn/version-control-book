...
To be able to use GitHub, you need to create an account:
...
::: {.callout-tip title="Find out more: Choosing a GitHub username" collapse="true"}
...
When authenticating with GitHub on a Mac, you may notice that the password doesn't show up in the command line as you type it.
This is normal behavior to ensure security. 
You can still enter your password even though it doesn't appear on the screen.
...
Note, that if you want to upload an existing repository from your computer, you should *not* initialize the repository on GitHub with any files, since your GitHub repository needs to be completely empty in order to synchronize your local repository.
...
If you want to upload your local repository to GitHub, ensure you create the GitHub repository **without initializing it with a `README` file** or any other files.
Initializing the repository with a `README` or any other files creates a commit in the GitHub repository. 
When you try to push your local repository, you may encounter conflicts because the histories of your local and remote repositories will differ.

To avoid this, create the GitHub repository empty. You can add a `README` and other files after pushing your local repository.
:::
...
To initialize an existing repository as a GitHub repository, you first need to create a new repository on GitHub, as explained above.
...
It's a standard name and we recommend to use it, but you could also choose any another name (for example, `github`) if you prefer. 
...
`git remote rename <old-name> <new-name>:` Renames a remote repository from old-name to a new-name.
