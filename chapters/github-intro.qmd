...
In this chapter, we will introduce the repository hosting platform [GitHub](https://github.com/), mainly because it is the most popular platform for Git repositories with 100 million users (as of January 2023, according to [Wikipedia](https://en.wikipedia.org/wiki/GitHub)).

### Why use remote repositories?

One of the main reasons to use remote repositories is for **collaboration**. 
They provide a central location where team members can collectively work on a project. 
...
!["In case of fire" by [Louis-Michel Couture](https://github.com/louim) (License: [MIT](https://github.com/louim/in-case-of-fire/blob/master/LICENSE); [Source on GitHub](https://github.com/louim/in-case-of-fire))](../static/git-in-case-of-fire){#fig-git-in-case-of-fire width="70%"}
...
::: {.callout-tip title="Choosing a GitHub username" collapse="true"}
...
::: {.callout-tip title="GitHub Student Developer Pack" collapse="true"}
...
Follow these steps to set it a Personal Access Token:
...
You can also choose to create a `README.md` file by checking the corresponding box.
If you want to upload an existing repository from your computer, you should not create a `README.md`, since your GitHub repository needs to be completely empty.
Finally, click the green **"Create repository"** button at the bottom of the page to complete the creation process.

::: {.callout-tip title="Public or private repository?" collapse="true"}
...
Let's assume that you have a local Git repository on your computer that you want to upload to GitHub.
To initialize an existing folder as a GitHub repository, you first need to create a new repository on GitHub, as explained above.

To add a remote repository to your folder, navigate to your folder using the command line and use the `git remote` command.
...
After running this command, your local Git repository will be aware of the remote repository, and you can synchronize changes between your local repository and the remote repository on GitHub.
The term "origin" is a conventionally used name for the default remote repository. It's a standard name and we recommend to use it, but you could also choose any another name (for example, `github`) if you prefer. 

Your are now able to upload your files to the remote repo using `git push`.

```{zsh filename="Code"}
git push -u origin main
```

This uploads the local branch named "main" to a remote repository called "origin" while setting it as the upstream branch.
...
### {{< fa clone >}} Cloning a repository from GitHub
...
That means if you have made changes in your remote repository to a branch called e.g. `feature`, `git fetch` will leave your local `feature` branch as it is, but will update your remote tracking branch called: `origin/feature`.
...
Th `git push` command is like a reversed `git pull`.
...
The command above pushes your local commits to the remote repository called `origin` on the branch `main`.
