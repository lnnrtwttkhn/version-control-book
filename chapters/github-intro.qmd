...
abstract: |
  Introduction to remote repositories and GitHub in particular
---
...
A remote repository in Git is a version of your project that is hosted on a server, mostly on the internet.
Unlike your local repository, which is on your personal machine, a remote repository is a shared resource that allows multiple contributors to collaborate on a project. 
...
If a contributor's local machine fails or their files get corrupted, they can always clone a fresh copy of the project from the remote repository.
This ensures that the project's codebase is not dependent on a single person's local environment.
...
As discussed in the [setup chapter](setup.qmd), you can set an username and an email for Git using:
...
Your Git username does not have to match your GitHub username, but it does make a lot of sense to set your Git username as your GitHub username.
This is so it is clear who made which contributions when viewing the repository's history or pull requests on GitHub.

Your set email has to match you used to create your GitHub account, so you can login and authenticate yourself to GitHub. 
...
If you would rather use the SSH protocol for authentification, check out the [SSH documentation](https://docs.github.com/en/authentication/connecting-to-github-with-ssh) on Github.com.
...
To add your folder as a remote repositor, navigate to your folder using the command line and use the `git remote` command.

```{bash}
git remote add origin https://github.com/yourusername/repositoryname.git
```
This command is essentially saying: "Add a remote repository named 'origin,' and the location (URL) of that remote repository is 'https://github.com/yourusername/repositoryname.git'." After running this command, your local Git repository will be aware of the remote repository, and you can push and pull changes between your local repository and the one on GitHub.
...
It enables you to make changes independently, create new branches, and push modifications to your forked repository. You can also submit changes to the original repository through pull requests.
...
You can test this function by editing a file in your recipe repo on GitHub and then use `git pull` to update your local file with the change made on GitHub.
...
You can add, modify, or delete content as needed. As you make changes, GitHub automatically tracks your modifications in the editor, displaying them as "changes not staged for commit".
...
The first part of the message indicates the range of commits that were fetched and merged. In this example, it suggests that Git is updating from commit `abc123` to commit `def456`.
The "Fast-forward" message indicates that the local branch could be updated by simply moving the branch pointer forward, incorporating the new commits from the remote branch. 
...
The last line of the message provides a summary of the changes. It states that one file was changed, with one line inserted and one line deleted.
...
To view your remote tracking branches, you can use `git branch -r` After fetching, you can use `git switch` to review the changes on these branches.
...
This pushes your local commits to the remote repository called: `remote` on the branch `main`.
After a successfull push, you will get a message like
