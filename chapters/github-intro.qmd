...
--rebase: Instead of the default "merge" strategy, use the "rebase" strategy to apply your local changes on top of the updated remote branch.
This keeps a cleaner commit history by avoiding unnecessary merge commits.
...
Instead Git will update all your local tracking branches to the remote branche.
...
`git fetch` is the safer option compared to `git pull` since it retrieves the changes from the remote repository wirhout making any changes to your local working directory and staging area.
