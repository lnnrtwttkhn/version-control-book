...
-   Login to the client using your GitHub Account and view your Receipe-repository
-   Add two receipes in the same file, stage and commit only one of them, using a GUI
-   Open your Receipe-repository in R-Studio, edit a file and commit your changes
...
As this book primarily emphasizes teaching Git through the command-line interface, it will only feature a shirt introduction, not extensive tutorials for GUI tools.
...
In this section, we will focus on GitHub Desktop and GitKraken, although many more options exist.
...
Once logged in, you can select a repository to dp work on.
Either from Github.com, or on a local repository.
...
The interface is also pretty simple and user-friendly.
...
To do this in GitKraken, change a file, then click on your last commit in the GitKraken GUI. You will see a list of unstaged files with changes.
...
To stage multiple lines at the same time, highlight the ones you want to stage and rightclick.
...
Even though it is not a complete GUI, RStudio, the most pupular environment for R programming, offers Git integration to simplify version control within your projects.
...
![.](../static/Rstudio-newproject.png){#fig-Rstudio_start} After cloning a repository, or just opening a local git-initialized folder, you do things like committing, pushing or switching branches using buttons in Rstudio, instead of using the terminal.
