...
It is identified by a unique name and a file extension, which indicates the file's format or type (for example, `file.txt`). 
...
A **directory** is a more technical term, while **folder** is a user-friendly term for the same concept. 
...
![Terminal icon in the macOS dock](../static/mac-dock.png){#fig-terminal width="60%"}
...
For Linux and macOS users, you can install it via your package manager (e.g., brew install tree on Mac, if Homebrew is installed) to achieve similar functionality
:::
...
::: {.callout-tip collapse="true" title="Finding the Tilde (~)"}
- **macOS**: 
You can type the tilde (`~`) symbol by pressing the `option` key and the `N` key simultaneously, then pressing the `spacebar`.

**Windows (German Keyboard Layout)**:
On a Windows PC with a German keyboard layout, you can type the tilde (`~`) by holding the `Alt Gr` key and pressing the `+` key.
:::
...
Once you've typed a portion of the command or path, press the {{< kbd ⇥ Tab >}} key.
...
::: {.callout-warning title="Be cautios when modifying hidden files!"}
...
To list *only* hidden files, see the command in @tip-only-hidden.
...
Make sure that the project folder is present by using the `ls` command, and then move into it by using the `cd` command.
...
The `echo` command is a simple and versatile tool used to display text or output it to files. 
...
For example, `echo "Current date: $(date)"` will display the current date.
...
To do this, you can first `cd` into the `/vegan` subfolder and then use the `..`  (for details on `..`, see the [section above](#dot-dot-..)) in combination with the `mv` command:
...
It's fundamental for using Git and offers advantages like automation, flexibility, and access to advanced features.

**Terminology:** 
Key terms include:

**File**: A unit of data storage, identified by a name and extension (e.g., `file.txt`).

**Folder/Directory:**
Containers that organize files in a hierarchical structure.

**CLI (Command-Line Interface):**
The text-based interface for entering commands.

**Terminal:** 
The program that provides access to the CLI.

**Shell:**
The software that interprets commands typed into the terminal.

**Navigating the File System:**
You can use commands to move between directories, list contents, and understand the directory structure. 
The command line allows for precise and efficient file management.

**Managing Files and Folders:**
The command line lets you create, move, rename, and delete files and folders with simple commands. 
It supports operations on multiple files using wildcards and other powerful tools.

**Writing and Viewing Text:**
You can write text directly into files or display file content in the terminal. 
This is useful for quickly updating or viewing file data without opening a separate text editor.

**Efficiency Tools:**
The command line offers tools like autocompletion, command history, and shortcuts (`~`, `.`) to speed up your workflow. It also supports combining commands for more complex operations.
