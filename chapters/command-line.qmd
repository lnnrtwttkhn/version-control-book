...
Even though many [Graphical User Interfaces](gui.qmd) exist, Git is fundamentally a command-line tool.
This is why comfort with the command line a good basis for learning Git.
...
Let's say we want to move the `recipes.txt` file to the `/vegan` folder.
...
To move the `recipes.txt` file, the command would look like this:
...
Suppose we don't like the folder name `vegan` because `recipes.txt` actually contains meat recipes.
