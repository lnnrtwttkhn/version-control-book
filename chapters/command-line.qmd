...
Parent directories contain child directories, creating a parent-child relationship.
The **root directory** is the highest level in the hierarchy.
On Unix-like systems (including macOS), the root directory is denoted by `/`.
...
On Mac OS the root directory is denoted by `/`, which is also used as the directory separator.
...
Note that if a path of yours contains a space, e.g.: `/Users/Your User/Downloads/Directory with Space/` it needs to be enclosed in qoutes, to be recognized by the command line.
If you attempt to use this path in a command without quotes, the command interpreter would see it as three separate arguments

Since in command-line interfaces, spaces are typically used to separate different arguments or parameters.
Navigating to this absolute path, which contains spaces would look like this:

```{bash}
cd '/Users/Your User/Downloads/Directory with Space/'
```
...
While using the command line, instead of typing out the full command or path, you can press the `Tab` key to automatically fill in the rest.

Begin typing a command or the initial letters of a file or directory.
E.g.:
...
Once you've typed a portion of the command or path, press the `Tab` key.
...
If there are multiple possibilities, pressing `Tab` again will cycle through the available options.
Autocomplete also works for system commands. 
For example, typing `ls` and pressing `Tab` might complete it to `ls -l` or `ls -a`.
Autocomplete also works for Git commands, that will be introduced later in this book.

### Command history

Using the `Up` and `Down` arrow keys on your keyboard allows ypu to quickly access and reuse commands without retyping them.
...
The primary benefit of thos is time efficiency. 
...
E.g. You can use the `open` command together with the `--help` flag:
...
To view and/or edit files, you can use your preferred text editor or also use the command line. 
...
These commands will open the specified file in the default associated application.
...
For instance, to add text to the file.txt file:
...
If you want to view the content of a file without opening it, you can do so in the command line using the `cat` command.
...
The first column (`-rw-r--r--, -rw-r--r--@`) represents the file permissions. In Unix-like systems, it shows who can read, write, or execute the file. In this case, the files are readable and writable by the owner (`rw-`), readable by the group (`r--`), and readable by others (`r-`).

The second column (`1`) indicates the number of hard links to the file.

The third column (`konradpagenstedt`) is the username of the file's owner.

The fourth column (`staff`) is the group associated with the file.

The next columns (`Aug 11 13:10, Jan 3 11:29, Jan 5 13:34`) display the date and time when the file was last modified.

The last column (`_affiliations.yml, _authors.yml, _metadata.yml, _quarto.yml, _variables.yml`) is the name of each file.
:::
...
Let's say you want to create a new directory, move into that directory, and then list the files in that directory, all in one go. 
...
This will: Create a new directory called `new_directory`,  Change the current working directory to `new_directory` and list the files in the new directory. 
