...
Let's say that you try to rename the `vegan` folder to `meat` but you are *inside* the `vegan` folder (check with `pwd`), you will receive an error like this:
...
Here, we present two simple approachs using the tools `curl` and `wget`.
