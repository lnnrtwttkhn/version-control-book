...
- Explain how the shell relates to the keyboard, the screen, the operating system, and users’ programms.
...
If the output is `/bin/zsh` or a similar path, it means your default shell is set to Zsh. This indicates that you are using Zsh as your shell.
...
A dropdown menu will appear; choose "About This Mac."
...
JupyterHub is a web-based platform that allows multiple users to access **Jupyter Notebook**, which is an interactive computing environment, on a shared server.
...
Once you are logged in, you should be able to view the remote environment and create files using different programms like R, Python or Java.
...
### Introduction
You might be familiar with using your mouse to click through folders, or perhaps you use arrow keys and clicks in the Finder (on Mac) or File Explorer (on Windows) to manage your files.

The command line offers an different, less intuitive, but very efficient and precise way to interact with your computer's file system. 
...
### List hidden files
Hidden files, often starting with a dot in their names, are files that remain mostly concealed from the regular view in GUI´s.
They can serve different purposes, such as storing configuration settings, sensitive data, or temporary files.
Accessing hidden files can be done through the command line or by changing settings in the file manager to make them visible.
...
```
**Warning:** 
When you use the rm command to delete files, they are removed entirely from your system and cannot be retrieved from a 'Recycle Bin' or 'Trash' as when you delete files using GUI´s.
