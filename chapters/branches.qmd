...
image: ../static/brancheschapter.png
abstract: |
...
Git makes it easy to switch between branches and handle conflicts when merging.ƒ
...
You can check how your branch, that you are using, is named by using `git branch`.
...
The asterisk (*) before the branch name signifies the active or current branch in your Git repository.

In Git, you can have multiple branches in your project, and this command helps you see the list of branches present. 
The one with the asterisk is the branch where your working directory is currently positioned. 
In this case, "main" is the active branch. If there were other branches listed without an asterisk, it means they exist, but you are not currently on those branches; you are on "main."

## Creating a new branch

To create a new branch, simply use `git branch` followed by the wished branch name.
E.g.:
...
Your newly created branch will show up, but your active branch should stil be `main`.
...
### Error switching branches

Git is designed to prevent you from switching branches if there's a risk of overwriting your changes.
That means Git will not allow you to switch branches if you have uncommited changes on your branch.
If you try to switch branches with uncommited changes you will get an error message similiar to:
...
It is also possible to temporarily stash your changes using `git stash` or delete your changes using `git reset` which will both be covered in later chapters.
...
In other words, there are no new changes in the `feature` branch that need to be merged because your current branch already contains all the changes from `feature`.
...
The output reveals a "Fast-forward" merge operation, where changes from commit 555ba0c to 994bb8d are incorporated. T
he common starting point is 555ba0c, and 994bb8d marks the latest commit in the merged branch.
A "Fast-forward" merge, the simplest form, smoothly updates your position to the latest changes without creating a new bookmark. This linear integration avoids diverging paths.
...
If you only work locally, without a remote repository, you can avoid merge conflicts easier, and might not even need different branches.
If you start to collaborate with other, it may not be possible to entirely avoid merge conflicts.

Employing good development practices can minimize their occurrence.
