...
![Creating a branch using Rstudio](../images/rstudio_newbranch.png){#fig-rstudio_newbranch}
...
![Switching branches using Rstudio](../images/rstudio_switchbranch.png){#fig-rstudio_switchbranch}
...
::: {.callout-note title="Is it possible to merge multiple branches with one command?" collapse="true"}

There is **no built-in Git command to merge all branches** into the current branch with a single command, primarily because mass-merging can be risky and prone to conflicts.
That said, it is technically possible using a script or loop that identifies all branches and merges them one by one.
However, since this approach is generally discouraged and can lead to complex merge issues, we do not include instructions in this guide.
If needed, you can find examples and tutorials online.
