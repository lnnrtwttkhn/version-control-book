...
- Manage project versions effectively using tags for smooth releases
...
Fundamentally, tags are used to mark particular points in a project's history, usually to signify important milestones or releases.
A tag essentially points to a specific commit in the version control history.
...
Tags are named in a way that is meaningful and easy to understand.
...
One primary use of tags is to mark releases or significant milestones in the development process.
They provide a snapshot of the codebase at a specific point in time.
...
Unlike annotated tags, they carry minimal metadata, consisting only of the tag name and the hash of the associated commit. Lightweight tags are created using `git tag` followed by the tag name.
E.g.:
...
(If you do not use the `-m` flag, your editor of choice will open up.)
...
Creating annotated tags is a common practice for versioning releases in software projects, as the associated metadata helps provide a clear and detailed history of the project at specific points in time.

::: {.callout-tip collapse="true"}
### Common `git tag` flags

`-a` or `--annotate`:
Creates an annotated tag, which includes additional metadata like tagger information and a tagging message.

`-m` or `--message`:
Specifies the tagging message directly in the command. 
...
Now you are on the tag, and your working directory reflects the state of the code at the time the tag was created.
...
A Git tag essentially "tags" a particular snapshot of your project, making it easy to reference and distribute as a release.
By creating a GitHub release, you've established a clear point of reference for users of your program and/or collaborators.


Creating a GitHub release involves several steps, including associating the release with a Git tag, providing release notes, and attaching relevant assets. Here's a step-by-step guide:
...
3. Choose an existing Git tag or create a new one. Enter the tag version in the "Tag version" field.
Enter a meaningful title for your release. This title will be displayed on the GitHub Releases page.
In the "Describe this release" field, provide release notes. This can include details about new features, bug fixes, and any other relevant information.
...
4. If you're creating a draft release, you can save it as a draft by clicking on the "Save draft" button. Draft releases are not visible to the public.
...
## Zenodo
[Zenodo](https://zenodo.org) is an open-access digital repository designed to preserve and share research outputs. 
...
For a pictured guide on how to upload a GitHub repository to Zenodo, you check out [this presentation](https://www.lib.uiowa.edu/data/files/2022/10/Share_and_Preserve_Your_Code_20220728.pdf) from the University of Iowa.
If you are just testing, you should use the [Zenodo sandbox](https://sandbox.zenodo.org), to not create an unnecessary real DOI which is hard to delete.
