...
To exit the Git log view and return to the regular terminal, simply press the `q` key. 
...
It is standard to start the title with an **imperative verb** to indicate the purpose of the commit, for example, "add", "fix" or "improve" instead "added", "fixed" and "improved".
This is standard practice since writing in the present tense tells someone what applying the commit will do, rather than what you did.
Of course yopu can also choose to use the past sense, if you think it makes more sense for your project.
...
It does not have a file extension like e.g. `txt` and is simply named `.gitignore`.
...
Renaming files in Git  can be done using the `git mv command`.
This command not only renames the file but also stages the change for you, so the rename is ready to be committed. 
This method is preferable to renaming the file manually, as it allows Git to track the change seamlessly and avoids the need for you to stage both the removal of the old file and the addition of the new file separately.
...
```
After running this command, Git stages the file rename, and you can commit the change like any other.
