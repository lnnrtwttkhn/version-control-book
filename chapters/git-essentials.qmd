...
::: {.callout-tip title="Dynamic `.gitignore maker`: Helps decide what files belong in your`.gitignore` file" collapse="true"}

When setting up your `.gitignore` file, [.gitignore.io](gitignore.io) can help you decide what files to include in it.
It is a free website that tells you which files and file extensions should be part of your `.gitignore`.
It dynamically adjusts the proposed `.gitignore` file depending on your programming language, operating system and development environment. 
This gives you a great starting point and helps with remembering all the files you would want to ignore.
You can either use the [web service](.gitignore.io) or you can set it up from the command line using their [documentation](https://docs.gitignore.io/install/command-line).
