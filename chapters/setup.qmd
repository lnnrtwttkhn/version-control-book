...
::: {.callout-warning title="Learning Git through RStudio? Create an RStudio project first!" collapse="true"}

An RStudio project is a folder that contains all files related to your analysis, such as code, data, and documentation.
It serves as a complete research object, making your work organized and reproducible.

To create a new project in RStudio:

1. Go to **File** → **New Project** → **New Directory**.

	![](../images/rstudio-new-directory.png){#fig-rstudio-new-directory}

1. In the **Project Type** screen, select **New Project**.

	![](../images/rstudio-new-project.png){#fig-rstudio-new-project}

1. In the **Create New Project** screen, enter a project name (e.g., `my-project`).
Make sure **Create a git repository** is checked.
Click **Create Project**.

	![](../images/rstudio-create-new-project.png){#fig-rstudio-create-new-project}

RStudio will create a new folder for your project and set R's working directory to that location.
You will see two files in the new project folder:

![](../images/rstudio-git-files.png){#fig-rstudio-git-files}

- **.gitignore** – Lists files and folders that Git should ignore (for example, sensitive data or files that do not need version control).
See [`.gitignore` configuration](git-essentials.qmd#ignoring-files-and-folders-.gitignore) for more details.
- **my-project.Rproj** – Stores configuration information for the RStudio project.

Don't worry about these files for now.
:::
...
Especially if you're new to Git (though not only then!), you might worry that something will go terribly wrong, that you'll lose files beyond recovery or even "break your computer".
While we can't guarantee that this will *never* happen (if you try hard enough, it's always possible to break things), it's quite unlikely that anything serious will go wrong.
In this section, we've collected tips and tricks to help make your Git journey as smooth as possible.
...
This way, you are protected from accidentally creating a Git repository in your home directory.

::: {.callout-tip title="Recommendation: Use this guardrail *shortly before* you create your first Git repository" collapse="true"}
...
::: {.callout-warning title="Potential interference with other applications (like Psychopy or DataLad)" collapse="true"}
Note that some applications on your computer (like [PsychoPy](https://www.psychopy.org/) and [DataLad](https://www.datalad.org/)) will scan your directory hierarchy for existing Git repositories and display an error message when they detect the empty `.git` file you may have set up as a guardrail.
In this situation, we recommend simply removing the empty `.git` file in your home directory (and possibly re-creating it after you set up your project in the other application).
Remember that this is intended as a *helpful* guardrail and provides extra protection against accidentally creating a Git repository in your home directory.
However, if it proves more annoying than helpful, feel free to remove it.
:::
