...
It's also possible to include the `make images` as a project script in the `pre-render` script.
This would re-execute `make images` before every render.
...
For example, to include the link to the Nextcloud folder that stores images contents (images etc.) as a variable in a document, use `{{{< var links.nextcloud >}}}`.
