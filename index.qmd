...
testimonials <- list(
  ...
  "Sensible structure, content explained in an easy-to-understand manner; this allowed one to work through it at their own pace during the sessions or review the content between sessions."
)

# Create the carousel with custom CSS for smaller text
carousel <- tags$div(
  id = "testimonialCarousel",
  class = "carousel slide",
  `data-ride` = "carousel",
  `data-interval` = "5000",  # Automatically change slides every 5 seconds
  
  # Add custom CSS for font size
  tags$style(HTML("
    .testimonial p {
      font-size: 14px;  /* Adjust this value for smaller or larger text */
    }
  ")),
  ...
  tags$div(
    ...
    # Loop through the testimonials and generate carousel items
    lapply(seq_along(testimonials), function(i) {
      tags$div(
        class = ifelse(i == 1, "carousel-item active", "carousel-item"),
        tags$div(
          class = "testimonial",
          tags$blockquote(class = "blockquote", tags$p(testimonials[[i]]))
        )
      )
    })
  )
